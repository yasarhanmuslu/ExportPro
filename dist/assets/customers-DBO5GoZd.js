import{r as p,s}from"./navbar-Dg2HCFHJ.js";import{r as E}from"./auth-CaJjsXSj.js";let d=[];document.addEventListener("DOMContentLoaded",async()=>{await E()&&(await p("customers"),await m(),f())});async function m(){try{const{data:{session:n}}=await s.auth.getSession();if(!n)return;const{data:e,error:t}=await s.from("customers").select("*").order("country",{ascending:!0}).order("company_name",{ascending:!0});if(t)throw t;d=e,I(e),y(e)}catch(n){console.error("Müşteri listesi çekilemedi:",n.message),alert("Müşteri verileri yüklenirken hata oluştu.")}}function y(n){const e=document.getElementById("customers-list-container");if(e.innerHTML="",n.length===0){e.innerHTML=`
            ...
            </div>`;return}const t={};n.forEach(a=>{const o=a.country.trim();t[o]||(t[o]=[]),t[o].push(a)}),Object.keys(t).forEach(a=>{const o=t[a].length,r=document.createElement("div");r.className="bg-[#FBF8F1]/40 border border-[#EFEAE0] rounded-xl overflow-hidden shadow-md",r.innerHTML=`
            <div class="bg-[#FBF8F1]/80 px-6 py-4 flex items-center justify-between cursor-pointer border-b border-[#EFEAE0]/60 select-none toggle-group-btn">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold text-[#1C1A17] tracking-wide">${a.toUpperCase()}</span>
            ...
            <div class="custom-table-container border-0 rounded-none transition-all duration-200">
                <table class="custom-table">
                    ...
                    <tbody>
                        ${t[a].map(i=>`
                            <tr>
                                <td class="font-medium text-[#1C1A17]">${l(i.company_name)}</td>
                                <td class="text-[#6B655B]">${l(i.contact_name||"—")}</td>
                                <td>
                                    <div class="text-xs">${l(i.email||"—")}</div>
                                    <div class="text-xs text-[#968B7A]">${l(i.phone||"")}</div>
                                </td>
                                <td>
                                    <span class="px-2.5 py-1 rounded-md text-xs font-medium border ${x(i.client_group)}">
                                        ${i.client_group||"Standart"}
                                    </span>
                                </td>
                                <td>
                                    <span class="px-2 py-0.5 rounded text-xs font-semibold ${w(i.status)}">
                                        ${i.status||"Aktif"}
                                    </span>
                                </td>
                                <td class="text-[#6B655B] text-xs">${new Date(i.created_at).toLocaleDateString("tr-TR")}</td>
                                <td class="text-right">
                                    <button class="btn-edit-trigger text-xs bg-[#FBF8F1] hover:bg-slate-700 border border-[#E4DDCE] hover:border-slate-600 px-3 py-1.5 rounded-md text-blue-400 transition-colors" data-id="${i.id}">
